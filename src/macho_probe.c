...
TODO: Document this section extensively.
      Improve organization of text
*/

void macho_print_info(const char *path, FILE *log) {
    int fd = open(path, O_RDONLY);
    if (fd < 0) { fprintf(log, "[macho] open %s failed: %s\n", path, strerror(errno)); return; }
    uint8_t buf[4096];
    ssize_t n = read(fd, buf, sizeof(buf));
    if (n < 4) { fprintf(log, "[macho] %s: too small\n", path); close(fd); return; }
    ...
    if (magic == FAT_CIGAM || magic == FAT_MAGIC) {
        ...
        fprintf(log, "[macho] %s: 64-bit Mach-O, cpu=%x, type=%x, ncmds=%u\n", path, mh->cputype, mh->filetype, mh->ncmds);
    } else if (magic == MH_MAGIC || magic == MH_CIGAM) {
        struct mach_header *mh = (struct mach_header*)buf;
        fprintf(log, "[macho] %s: 32-bit Mach-O, cpu=%x, type=%x, ncmds=%u\n", path, mh->cputype, mh->filetype, mh->ncmds);
    } else {
...
int macho_patch_first_byte(const char *path, uint8_t new_byte, FILE *log) {
    ...
    if (fd < 0) { fprintf(log, "[patch] open %s failed: %s\n", path, strerror(errno)); return -1; }
    uint8_t orig;
    if (read(fd, &orig, 1) != 1) { fprintf(log, "[patch] read %s failed\n", path); close(fd); return -1; }
    lseek(fd, 0, SEEK_SET);
    if (write(fd, &new_byte, 1) != 1) { fprintf(log, "[patch] write %s failed\n", path); close(fd); return -1; }
...
void macho_compare_dirs(const char *dir1, const char *dir2, FILE *log) {
    DIR *d1 = opendir(dir1);
    DIR *d2 = opendir(dir2);
    if (!d1 || !d2) { fprintf(log, "[compare] Could not open directories\n"); if (d1) closedir(d1); if (d2) closedir(d2); return; }
    struct dirent *e1;
    while ((e1 = readdir(d1)) != NULL) {
        ...
        struct stat st1, st2;
        if (stat(path1, &st1) == 0 && S_ISREG(st1.st_mode) && stat(path2, &st2) == 0 && S_ISREG(st2.st_mode)) {
            ...
            if (fd1 >= 0 && fd2 >= 0) {
                ...
                if (n1 > 4 && n2 > 4 && *(uint32_t*)buf1 == *(uint32_t*)buf2) {
                    if (memcmp(buf1, buf2, 32) != 0) {
                        fprintf(log, "[compare] %s differs in Mach-O header\n", e1->d_name);
                    }
                }
                close(fd1); close(fd2);
            }
...
void macho_extract_dir(const char *dir, FILE *log) {
    DIR *d = opendir(dir);
    if (!d) { fprintf(log, "[extract] Could not open %s\n", dir); return; }
    struct dirent *e;
    char path[512];
...
void macho_deep_probe(const char *dir, FILE *log) {
    DIR *d = opendir(dir);
    if (!d) { fprintf(log, "[deep] Could not open %s\n", dir); return; }
    struct dirent *e;
    char path[512];
    while ((e = readdir(d)) != NULL) {
        if (e->d_name[0] == '.') continue;
        ...
        if (stat(path, &st) == 0 && S_ISREG(st.st_mode)) {
            ...
            if (fd >= 0) {
                ...
                if (n > 4) {
                    uint32_t magic = *(uint32_t*)buf;
                    if (magic == FAT_CIGAM || magic == FAT_MAGIC || magic == MH_MAGIC_64 || magic == MH_CIGAM_64 || magic == MH_MAGIC || magic == MH_CIGAM) {
...
void macho_list_architectures(const char *path, FILE *log) {
    ...
    if (magic == FAT_MAGIC || magic == FAT_CIGAM) {
        ...
        fprintf(log, "[macho] %s: Universal binary with %u architectures\n", path, narch);
        struct fat_arch *arch = (struct fat_arch*)(buf + sizeof(struct fat_header));
        for (uint32_t i = 0; i < narch; ++i) {
            fprintf(log, "  arch %u: cputype=0x%x, cpusubtype=0x%x\n", i, ntohl(arch->cputype), ntohl(arch->cpusubtype));
            arch++;
        }
    } else if (magic == MH_MAGIC_64 || magic == MH_CIGAM_64) {
        struct mach_header_64 *mh = (struct mach_header_64*)buf;
        fprintf(log, "[macho] %s: Single arch, cputype=0x%x, cpusubtype=0x%x\n", path, mh->cputype, mh->cpusubtype);
    } else {
        fprintf(log, "[macho] %s: Not a Mach-O\n", path);
    }
    close(fd);
...
static void macho_probe_cb(const char *path, void *logptr) {
    ...
    if (read(fd, buf, 4) == 4) {
        uint32_t magic = *(uint32_t*)buf;
        if (magic == FAT_MAGIC || magic == FAT_CIGAM || magic == MH_MAGIC_64 || magic == MH_CIGAM_64 || magic == MH_MAGIC || magic == MH_CIGAM) {
            fprintf(log, "[deep] Mach-O found: %s\n", path);
            macho_print_info(path, log);
        }
    }
